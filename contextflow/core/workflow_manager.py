...
class WorkflowManager:
    ...
    def _has_action_words(self, text: str) -> bool:
        ...
        action_words = [
            ...
            'updated', 'modified', 'changed', 'improved',
            'refactored', 'optimized', 'enhanced',
            'tested', 'validated', 'verified',
            'documented', 'wrote', 'drafted'
        ]
    ...
    def _get_session_statistics(self) -> Dict[str, Any]:
        ...
        last_session = None
        if log_files:
            last_session = datetime.fromtimestamp(log_files[0].stat().st_mtime).strftime('%Y-%m-%d %H:%M')
    ...
    def _get_maintenance_status(self) -> Dict[str, Any]:
        ...
        if log_dir.exists():
            retention_cutoff = datetime.now().timestamp() - (self.config.workflow.session_log_retention_days * 24 * 60 * 60)
        ...
        if quick_context_file.exists():
            ...
            maintenance['context_refresh_needed'] = context_age_days > 7  # Refresh if older than 7 days
        else:
    ...
    def get_workflow_recommendations(self) -> List[str]:
        ...
        if not self.config.workflow.mandatory_session_updates:
            recommendations.append(
                "Enable mandatory session updates for better project tracking."
            )
