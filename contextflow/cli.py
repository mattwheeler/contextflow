...
@main.command()
@click.option("--template", "-t", default="software-development", help="Project template to use")
@click.option("--name", "-n", prompt="Project name", help="Name of the project")
@click.option(
    "--description", "-d", prompt="Project description", help="Description of the project"
)
...
@main.command()
@click.option("--quick", "-q", is_flag=True, help="Show quick context for immediate use")
@click.option("--refresh", "-r", is_flag=True, help="Refresh context from latest project state")
...
@main.command()
@click.option("--recent", "-r", is_flag=True, help="Show only recent session logs")
@click.option("--count", "-c", default=10, help="Number of logs to show")
    ...
    try:
        ...
        for log_file in log_files:
            try:
                ...
                summary = (
                    summary_match.group(1).strip()[:80] + "..." if summary_match else "No summary"
                )
...
def templates():
    ...
    try:
        ...
        for template_name, template_info in available_templates.items():
            ...
            table.add_row(
                template_name, template_info.get("description", "No description"), integrations
            )
